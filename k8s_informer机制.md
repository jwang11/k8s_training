...
- 添加事件处理handler（来自high level的业务controller）
```diff
...
+	// 创建processor的listener
	...
	if !s.started {
+		// informer的processor下可以有多个listener，对应多个high level的业务controller	
...
举个最简单的例子，看看Pod对象的Informer如何实现，其实是自动生成的（Code generated by informer-gen）
